...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const identity = await wallet.get('appUser2Org3');
        if (!identity) {
            console.log('An identity for the user "appUser2Org3" does not exist in the wallet');
            return res.status(400).json({ error: 'User identity "appUser2Org3" not found in the wallet. Please register the user.' });
        }

        // Create a new gateway for connecting to Org3's peer node.
        gateway = new Gateway();
        await gateway.connect(ccp, { wallet, identity: 'appUser2Org3', discovery: { enabled: true, asLocalhost: true } });
