...
async function main() {
    try {
        ...
        const userIdentity = await wallet.get('appUser2Org3');
        if (userIdentity) {
            console.log('An identity for the user "appUser2Org3" already exists in the wallet');
            return;
        }
        ...
        await wallet.put('appUser2Org3', x509Identity);
        console.log('Successfully registered and enrolled user "appUser2Org3" and imported it into the wallet');

    } catch (error) {
        console.error(`Failed to register user "appUser2Org3": ${error}`);
        process.exit(1);
    }
}
